...
/*
    Start
        declare 4 int count variables to count how many times a button is pressed
        declare 4 vector variables to put them in different positions
        Declare all of the variables from the top 4 times each for 4 different gamepads/controllers
        set all count variables to 0
        Load fonts
        Create for loops to count the buttons being pressed
        use edge detection for all buttons
        craete timer variable and only allow controllers to be connected if timer has been started
        draw all button counters on screen
    End
*/
...
namespace ButtonBash
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            //run if time4 or time10 is true
            if (time4 || time10)
            {
                //set all gamepad states
                ...
                //runs if controller 1 is connected
                if (pad1.IsConnected)
                {
                    //if the button "A" is pressed AND released then add 1 to a counter
                    if (oldpad.Buttons.A == ButtonState.Released && pad1.Buttons.A == ButtonState.Pressed)
                    {
                        acount++;
                    }
                    //if the button "B" is pressed AND released then add 1 to b counter
                    if (oldpad.Buttons.B == ButtonState.Released && pad1.Buttons.B == ButtonState.Pressed)
                    {
                        bcount++;
                    }
                    //if the button "X" is pressed AND released then add 1 to x counter
                    if (oldpad.Buttons.X == ButtonState.Released && pad1.Buttons.X == ButtonState.Pressed)
                    {
                        xcount++;
                    }
                    //if the button "Y" is pressed AND released then add 1 to y counter
                ...
                //if time4 and time10 equal to 240 then run code
                if (time4 && timer == 240)
                {
                    time4 = false;
                    time10 = false;
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            {
                //sets position for all the button counters
                if (pad1.IsConnected)
                {
